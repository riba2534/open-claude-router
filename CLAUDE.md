...
两条路径都汇入 `src/routes/messages.ts` 的 `forwardMessages()`。`src/utils/auth.ts` 里：`parseUpstreamConfig`（header 模式上游解析）、`parseUpstreamFromEmbeddedPath`（embedded-path 模式上游解析）、`parseUpstreamFormat`（协议变体），鉴权用 `checkServiceAuth`（header 模式 Bearer）和 `checkServiceAuthFromOcrTokenHeader`（embedded-path 模式 X-OCR-Token）。
...
unified  (anthropic transformer 把 request.thinking → result.reasoning，
           保留 cache_control 字段)
  ↓ scrubAnthropicOnlyFields  (always: 递归剥 cache_control)
  ↓ 分支:
      format=responses        → responses.transformRequestIn
                                  (消费 unified.reasoning 转成 Responses
                                   reasoning: { effort, summary } 入参)
      format=chat-completions → scrubChatCompletionsIncompatibleFields
                                  (剥 unified.reasoning，vanilla
                                   /chat/completions 拒收)
