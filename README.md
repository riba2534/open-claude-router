...
- **上游错误会重试吗**：会上报为可重试。上游返回任意非 2xx 时，服务保留原状态码和 Anthropic 错误体，并增加 `X-Should-Retry: true`；具体次数和退避由 Claude Code 客户端版本决定。服务自身不读取错误正文后修改请求体重发。可通过 `X-Upstream-Effort-Map: *=off` 或 `X-Upstream-Effort-Levels` 显式声明上游 effort 能力。
